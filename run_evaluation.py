...
original. The model expects 8kHz audio .wav files. Files with other 
...
def process_file(model, audio_file_name, out_file_name):
    ...
    Parameters
    ----------
    model : Keras model ~Keras model, which accepts audio in the size (1,timesteps).
    audio_file_name : STRING ~Name and path of the input audio file.
    out_file_name : STRING ~ Name and path of the target file.

    '''
     # read audio file with librosa to handle resampling and enforce mono
    in_data,fs = librosa.core.load(audio_file_name, sr=8000, mono=True)
